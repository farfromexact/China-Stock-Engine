...
6. GitHub Actions collection requires approved remote persistence. It may write normalized artifacts only to the repository named by `IFIND_DATA_REPOSITORY` when `IFIND_DATA_STORAGE_APPROVED=true` and a repository-scoped `STOCK_DATA_REPO_TOKEN` are configured; otherwise it fails before any iFinD request.
