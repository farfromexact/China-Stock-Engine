...
已接通有界iFinD财报/公告采集：当前映射仅包括归母净利润、实际披露日期及公告目录事实，实际范围和哈希见 `latest/supplemental_manifest.json`。每日开关启用后最多采集100只证券，先复用缓存；不是全市场覆盖。未实施：完整三表字段映射、预期修正、行业经营数据、历史估值分位、股息率和同业估值分位。新增收费字段必须继续验证指标、口径和权限，不能由已有行情替代。
